...
def neutralized(df,formatted_path,useLFLO=True,useSec=True,useBeta=False):
    ...
    xfunclist=list(filter(None,[betaxfunc if useBeta else None,LFLOxfunc if useLFLO else None,secxfunc if useSec else None]))
...
def getRiskRet_barra(excessret,riskfactor,perffilepath,formatted_path):
    ...
    if riskRet.index.max()<excessret.index.max():
        ...
        newriskfactor = pd.concat([newriskfactor, updatenewriskfactor.loc[:,newriskfactor.major_axis.max():,:].iloc[:,1:,:]],axis=1)
